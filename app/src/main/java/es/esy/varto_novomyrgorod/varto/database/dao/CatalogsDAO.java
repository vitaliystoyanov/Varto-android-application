...
public class CatalogsDAO implements DAOInterface<Catalog>, CatalogSchema {
    ...
    @Override
    public HashMap<Shop, Integer> update(List<Catalog> catalogs) {
        Cursor cursor = null;
        try {
            cursor = Database.getInstance(context).query(TAG_TABLE_CATALOG,
                    null, null, null, null, null, null);
            if (cursor.moveToFirst()) {
                int rowsAffected = Database.getInstance(context)
